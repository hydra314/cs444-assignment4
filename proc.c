...
found:
  ...
  *(uint*)sp = (uint)trapret;
  ...
  #ifdef PROC_TIME
  // Set variables to initialize process time and counting
...
scheduler(void)
{
  ...
  for(;;){
    ...
    target_val = random() % total_nice + 1;
    #endif  // LOTTERY_SCHED
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      #ifdef LOTTERY_SCHED
      runnable_nice += p->nice_value;
      if(runnable_nice <= target_val)
        continue;
      #endif  // LOTTERY_SCHED
    ...
    for (i = 0; i < NPROC; i++) {
        if (ptable.proc[i].state != UNUSED) {
            ...
            cprintf("%d\t%d\t%s\t%s\t%u\t%u-%s%u-%s%u %s%u:%s%u:%s%u\t%u\t%u\t%u"
                    ...
                    , add_leading_zero(ptable.proc[i].begin_date.month)
                    , ptable.proc[i].begin_date.month
                    , add_leading_zero(ptable.proc[i].begin_date.day)
                    , ptable.proc[i].begin_date.day
                    , add_leading_zero(ptable.proc[i].begin_date.hour)
                    , ptable.proc[i].begin_date.hour
                    , add_leading_zero(ptable.proc[i].begin_date.minute)
                    , ptable.proc[i].begin_date.minute
                    , add_leading_zero(ptable.proc[i].begin_date.second)
...
// Gets a process ID and an integer,
// sets the process's nice value to
...
// Gets a nice value and a program to execute
// Sets currently-running process to the inputted
// nice value and executes the program
int
nice(int new_nice, char *program)
{
